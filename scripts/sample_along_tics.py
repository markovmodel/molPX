...
parser.add_argument('-projects', type=str, nargs='+',
                    help='PDB code (or directory name) of the project')
...
parser.add_argument('--projection_dir', type=str, default='/group/ag_cmb/scratch/solsson/desresfeats/',
                    help='directory where the projected project lies')
parser.add_argument('--mdtraj_dir', type=str, default = '/group/ag_cmb/simulation-data/DESRES-Science2011-FastProteinFolding',
...
parser.add_argument('--out_dir', type=str, default=None, help='Directory to write results to. The default is '
                                                              'to write them in the projection_dir (ie. '
                                                              'one folder ABOVE the project"s directory)')
...
for proj in args.projects:
    ...
    fname_base =  ospath.join(args.out_dir,'%s.%s'%(proj, args.projection_startswith))

    # Inform about the project
    proj = ospath.join(args.projection_dir, proj)
    avail_tica_trajfiles= sort_nicely(glob(ospath.join(proj,args.projection_startswith)+'*.npz'))
    print('Doing pipeline for proj', proj)
        ...
        for coord in args.proj_idxs:
            ...
            if args.pdbs:
                ...
                if args.vmds:
                    ...
                    excluded_path =        np.argwhere(np.in1d(np.arange(cl.n_clusters), compact_path,        invert=True)).squeeze()[::-1]
                    excluded_path_sample = np.argwhere(np.in1d(np.arange(cl.n_clusters), compact_path_sample, invert=True)).squeeze()[::-1]
